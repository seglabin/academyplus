...
//        $this->Image('../img/entetebulletin.jpg', 20, 5, 170, 20);
        $this->Image('../img/entetebulletin.jpg', 5, 5, 200, 30);
//        $this->Image('../images/entete.png', 10, 5, 50, 20);
        // Saut de ligne
//        $this->Ln(9);
        //Functions::afficheBoiteMsg($_SESSION["libetablissemt"]);
//         $this->MultiCell(5, 5, utf8_decode( $_SESSION["libetablissemt"]), 0, 'C', FALSE);
...
//        if ($_SESSION["titre"] != '')
//            $this->MultiCell(150, 5, utf8_decode($_SESSION["titre"]), 0, 'J', FALSE);

//        $this->MultiCell(0, 35, utf8_decode('Etab : ' . $_SESSION["libetab"]), 0, 'L', FALSE);
//         $this->Ln(9);
        $this->MultiCell(0, 23, utf8_decode($_SESSION["libetablissemt"]), 0, 'C', FALSE);
        $this->MultiCell(0, -23, "", 0, 'L', FALSE);
        $this->MultiCell(0, 42, utf8_decode('Année Scolaire : ' . $_SESSION["libannescoBulletin"]), 0, 'R', FALSE);
        $this->Ln(18);
        $this->MultiCell(0, -45, "", 0, 'L', FALSE);
        $this->MultiCell(0, 9, '', 0, 'L', FALSE);
//        $this->Ln(15);
    }
...
// Pied de page
    function Footer() {
        ...
        // Numéro de page
        $this->Cell(0, 10, 'Copyright GSR' . date('Y'), 0, 0, 'R');
    }
     ...
     * $header
     */
    function FancyTabledd($header, $headerBD, $data, $w, $border = 0, $tabAlign, $fill = false) {
        // Saut de ligne
//        $this->Ln(5);
...
//        $this->Cell(array_sum($w), 0, '', 'T');
    ...
    function FancyTable($header, $headerBD, $data, $w, $border = 0, $tabAlign,$tbord = array(), $fill1 = true, $fill2 = false) {
//            print_r($data);
//        if(count($tbord)>$i) Functions::afficheBoiteMsg(count($tbord));
        /* Colors, line width and bold font */
//        $this->SetFillColor(69, 171, 82);
        $this->SetFillColor(150, 200, 182);
        $this->SetTextColor(0);
//        $this->SetDrawColor(209, 212, 207);
...
//                    $hentete = 5 * $nbl;
            $bord = (count($tbord)>$i)? $tbord[$i]:'UB';
            
            $this->Cell($w[$i], 7, utf8_decode($header[$i]), $bord, 0, $tabAlign[$i], $fill1);
        }
        $this->Ln();
...
//		$fill = false;
        ...
        foreach ($data as $row) {
            ...
            for ($i = 0; $i < count($headerBD); $i++) {
                //Cell($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='')
//                    $hentete = 5 * $nbl;
                $this->Cell($w[$i], $h, utf8_decode($row[$headerBD[$i]]), $border, 0, $tabAlign[$i], $fill);
            }
