...
//  $pdf->Cell(-90);
$pdf->SetFont('Arial', '', 12);
  $pdf->MultiCell(0, 10, utf8_decode('DATE : '.$date), 0, 'R', $fill);
$pdf->MultiCell(0, 20, utf8_decode($etab), 0, 'L', $fill);
$pdf->MultiCell(0, -20, utf8_decode($an), 0, 'R', $fill);
  $pdf->MultiCell(0, 40, utf8_decode($clas), 0, 'L', $fill);
  $pdf->MultiCell(0, -40, '', 0, 'R', $fill);
  $pdf->MultiCell(0, 60, utf8_decode('Apprenant : '.$apprenant), 0, 'L', $fill);
  $pdf->MultiCell(0, -60, utf8_decode('RECU N° : '.$numfacture), 0, 'R', $fill);
  $pdf->MultiCell(0, 80, utf8_decode('Montant payé : '.$total.'         Total payé : '.$totpai), 0, 'L', $fill);
  $pdf->MultiCell(0, -80, utf8_decode('Reste à payer : '.$rest), 0, 'R', $fill);
   $pdf->SetFillColor(150, 150, 100);
    $pdf->SetDrawColor(45, 45, 45);
        $pdf->SetLineWidth(.1);
  $pdf->MultiCell(0, 100, utf8_decode('Arrêté le présent reçu à la somme de '.$enlettr.' franc CFA.'), 0, 'C', $fill);
